...
export default function Success(props: SuccessProps) {
  ...
  const { isSellular, isLinkFromSellular } = router.query;
  const isSellularCalendar = isSellular === "true";
  const isLinkFromSellularCal = isLinkFromSellular === "true"
  const {
  ...
  const userIsOwner = !!((session?.user?.id && eventType.owner?.id === session.user.id) || (session?.user?.email && eventType.owner?.email === session.user.email));
  ...
  return (
    <div className={isEmbed ? "" : "h-screen"} data-testid="success-page">
      ...
      {isSellularCalendar && userIsOwner && !isEmbed && (
        <div className="mt-2 ml-4 -mb-4">
          <Link
            href={allRemainingBookings ? `/bookings/recurring${isSellularCalendar? `?isSellular=true` : ''}` : `/bookings/upcoming${isSellularCalendar? `?isSellular=true` : ''}`}
                        ...
                        {!props.recurringBookings && (
                          <span className="text-default inline">
                            <span className="underline" data-testid="reschedule-link">
                              <Link
                                href={`/reschedule/${seatReferenceUid || bookingInfo?.uid}${isSellularCalendar? `?isSellular=true` : ''}${isLinkFromSellularCal? `?isLinkFromSellular=true` : ''}`}
                                legacyBehavior>
                                {t("reschedule")}
                              </Link>
                            </span>
