...
function ConnectOrDisconnectIntegrationMenuItem(props: {
  ...
  item: any;
  handleDisconnect: (credentialId: number, item: any) => void;
  isSingleItem: boolean;
}) {
  const { type, credentialIds, isGlobal, installed, handleDisconnect, item ,isSingleItem} = props;
  const { t } = useLocale();
    ...
    return (
      <DropdownMenuItem>
        <DropdownItem
          color="transparent"
          onClick={() => {
            handleDisconnect(credentialId, item)
          }}
          disabled={isGlobal || isSingleItem}>
          {t("remove_app")}
        </DropdownItem>
      </DropdownMenuItem>
    );
...
const IntegrationsList = ({ data, handleDisconnect, variant }: IntegrationsListProps) => {
  ...
  const isSingleItem = data.items?.length === 1
  ...
  return (
    <>
      <List noBorderTreatment = {true} className = "mx-0">
        {data.items
                  ...
                  <div className="flex  justify-end">
                    <Dropdown modal={false}>
                      <DropdownMenuTrigger asChild>
                        <Button StartIcon={MoreVertical} variant="icon" color="secondary" className='text-default' />
                      </DropdownMenuTrigger>
                      <DropdownMenuContent>
...
const IntegrationsContainer = ({
  ...
  const {isSelConfSettings} = router.query
...
export default function InstalledApps() {
  ...
  const handleDisconnect = (credentialId: number, item: any) => {
    updateData({ isOpen: true, credentialId, item });
  };
