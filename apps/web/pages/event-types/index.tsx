...
const MobileTeamsTab: FC<MobileTeamsTabProps> = (props) => {
  const { eventTypeGroups } = props;  const router = useRouter();
  const { isSellular } = router.query;
  const isSellularCalendar = isSellular === "true";
  const tabs = eventTypeGroups.map((item) => ({
    name: item.profile.name ?? "",
    href: item.teamId ? `/event-types?teamId=${item.teamId}${isSellularCalendar? `&isSellular=true` : ''}` : `/event-types${isSellularCalendar? `?isSellular=true` : ''}`,
...
const Item = ({ type, group, readOnly }: { type: EventType; group: EventTypeGroup; readOnly: boolean }) => {
  ...
  return readOnly ? (
    ...
    <Link
      href={`/event-types/${type.id}?tabName=setup${isSellularCalendar? `&isSellular=true` : ''}`}
...
export const EventTypeList = ({ group, groupIndex, readOnly, types }: EventTypeListProps): JSX.Element => {
  ...
  return (
    <div className="bg-default border-subtle m-4 flex overflow-hidden border">
          ...
          return (
            <li key={type.id}>
              <div className="hover:bg-muted flex w-full items-center justify-between">
                <div className="group flex w-full max-w-full items-center justify-between overflow-hidden p-3">
                          ...
                          <Dropdown modal={false}>
                            <DropdownMenuTrigger asChild data-testid={"event-type-options-" + type.id}>
                              <Button
                                ...
                                className="ltr:radix-state-open:rounded-r-md rtl:radix-state-open:rounded-l-md text-sel-text-secondary"
                              />
                            </DropdownMenuTrigger>
                            <DropdownMenuContent>
                              {!readOnly && (
                                <DropdownMenuItem>
                                  <DropdownItem
                                    ...
                                    StartIcon={Edit2}
                                    onClick={() => router.push(`/event-types/${type.id}${isSellularCalendar? `?isSellular=true` : ''}`)}>
                                    {t("edit")}
                                  </DropdownItem>
                                </DropdownMenuItem>
                              )}
