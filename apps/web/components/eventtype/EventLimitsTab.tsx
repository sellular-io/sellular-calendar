  ...
  return (
    <div className="flex items-end justify-end">
      ...
      <Select
        ...
        className="mb-0 ml-2 h-[36px] w-full capitalize md:min-w-[150px] md:max-w-[200px]"
                  ...
                  <div
                    ...
                    {!periodTypeLocked.disabled && (
                      <RadioGroup.Item
                        id={period.type}
                        value={period.type}
                        className={`min-w-4 bg-default border-sel-secondary flex h-4 w-4 cursor-pointer items-center rounded-full border focus:border-1 focus:outline-none ltr:mr-2 rtl:ml-2  ${
                          watchPeriodType === period.type ? "border-emphasis" : ""
                        }`}>
                        <RadioGroup.Indicator className="after:bg-sel-main-primary relative flex h-4 w-4 items-center justify-center after:block after:h-2 after:w-2 after:rounded-full" />
                      </RadioGroup.Item>
                    )}
  ...
  return (
    <div className="mb-2 flex items-center space-x-2 text-sm rtl:space-x-reverse" key={limitKey}>
      ...
      <Select
        ...
        classNames={{control:() => cx(
          "bg-default border-default !min-h-[36px] h-[36px] text-sm leading-4 placeholder:text-sm placeholder:font-normal dark:focus:border-emphasis focus-within:outline-none focus-within:sellular-input-field-focus focus-within:ring-brand-default hover:border-emphasis border py-2 px-3 h-[36px] min-w-[120px]",
          disabled && "bg-subtle"
        )}}
      />
      {hasDeleteButton && !disabled && (
        <Button variant="icon" StartIcon={Trash} color="secondary" onClick={() => onDelete(limitKey)} />
      )}
    </div>
  );
};
        ...
        return (
          <ul ref={animateRef}>
            ...
            {currentIntervalLimits && Object.keys(currentIntervalLimits).length <= 3 && !disabled && (
              <Button color="transparent" StartIcon={Plus} onClick={addLimit}>
                {t("add_limit")}
              </Button>
            )}
          </ul>
