  ...
  return [
    {
      ...
      href: `/event-types/${eventType.id}?tabName=setup${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: LinkIcon,
      info: `${duration} ${t("minute_timeUnit")}`, // TODO: Get this from props
    },
    {
      name: "event_limit_tab_title",
      href: `/event-types/${eventType.id}?tabName=limits${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: Clock,
      info: `event_limit_tab_description`,
    },
    {
      name: "event_advanced_tab_title",
      href: `/event-types/${eventType.id}?tabName=advanced${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: Sliders,
      info: `event_advanced_tab_description`,
    },
    {
      name: "recurring",
      href: `/event-types/${eventType.id}?tabName=recurring${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: Repeat,
      info: `recurring_event_tab_description`,
    },
    {
      name: "apps",
      href: `/event-types/${eventType.id}?tabName=apps${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: Grid,
    ...
    {
      name: "workflows",
      href: `/event-types/${eventType.id}?tabName=workflows${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: Zap,
...
function EventTypeSingleLayout({
  ...
  const deleteMutation = trpc.viewer.eventTypes.delete.useMutation({
    onSuccess: async () => {
      ...
      await router.push(`/event-types${isSellularCalendar? `?isSellular=true` : ''}`);
  ...
  const EventTypeTabs = useMemo(() => {
    ...
    navigation.splice(1, 0, {
      name: "availability",
      href: `/event-types/${eventType.id}?tabName=availability${isSellularCalendar? `&isSellular=true` : ''}`,
      icon: Calendar,
      info:
    ...
    if (team) {
      navigation.splice(2, 0, {
        name: "assignment",
        href: `/event-types/${eventType.id}?tabName=team${isSellularCalendar? `&isSellular=true` : ''}`,
        icon: Users,
        hide: true,
    ...
    if (isManagedEventType || isChildrenManagedEventType) {
      ...
      navigation.push({
        name: "webhooks",
        href: `/event-types/${eventType.id}?tabName=webhooks${isSellularCalendar? `&isSellular=true` : ''}`,
        icon: TbWebhook,
  ...
  return (
    <Shell
      backPath={`/event-types${isSellularCalendar? `?isSellular=true` : ''}`}
      ...
      <Suspense fallback={<Loader />}>
        <div className="flex flex-col xl:flex-row xl:space-x-4">
          ...
          <div className="w-full ltr:mr-2 rtl:ml-2">
            <div
              className={classNames(
                "bg-default border-subtle  mt-4 mr-4 xl:mt-0",
                disableBorder ? "border-0 " : "p-2 md:border md:p-4"
              )}>
              {children}
            </div>
          </div>
