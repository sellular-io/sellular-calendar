...
const EventTypeScheduleDetails = ({
  ...
  return (
    <div className="border-default space-y-4 border px-4 pb-4">
            ...
            <li key={day} className="my-6 flex border-transparent last:mb-2">
              <span
                className={classNames(
                  ...
                  !isAvailable ? "text-subtle line-through" : "text-emphasis"
                )}>
                {day}
              </span>
              {isLoading ? (
      ...
      <div className="flex flex-col justify-center gap-2 sm:flex-row sm:justify-between">
        ...
        {!!schedule?.id && !schedule.isManaged && (
          <Button
            href={`/availability/${schedule.id}${isSellularCalendar? `?isSellular=true` : ''}`}
            disabled={isLoading}
            color="minimal"
