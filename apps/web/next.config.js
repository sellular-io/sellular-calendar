...
const nextConfig = {
    ...
    return [
      {
        ...
        headers: [
          {
            key: "X-Frame-Options",
            value:"ALLOW-FROM *.sellular.com *.sellulardev.io"
          },
        ],
      },
      {
  ...
  async redirects() {
    const redirects = [
      ...
      ...['/apps', '/apps/categories', '/apps/categories/:slug*', '/teams', '/workflows','/insights','/settings', '/settings/:slug*'].map(source => ({ source, destination: '/404', permanent: true,}))
    ];
