    ...
    <>
      <RadixToggleGroup.Root
        ...
        className={classNames(
          "min-h-9 border-default relative inline-flex gap-0.5 border p-1 bg-header",
          props.className,
          isFullWidth && "w-full"
        )}>
        {options.map((option) => (
          <OptionalTooltipWrapper key={option.value} tooltipText={option.tooltip}>
            <RadixToggleGroup.Item
              ...
              value={option.value}
              className={classNames(
                "aria-checked:bg-sel-main-primary aria-checked:text-inverted aria-checked:font-semibold text-xs relative px-3 py-1 leading-tight transition-colors",
                option.disabled
