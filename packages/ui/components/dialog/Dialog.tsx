...
export const DialogContent = React.forwardRef<HTMLDivElement, DialogContentProps>(
  ({ children, title, Icon, enableOverflow, type = "creation",...props }, forwardedRef) => {
    ...
    const disableOverlay = isSelSettings === 'true' || isSelConfSettings === 'true'
    return (
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className={`fadeIn fixed inset-0 z-50 ${disableOverlay ? '' :  'bg-neutral-800 bg-opacity-50'} transition-opacity dark:bg-opacity-50 `} />
        <DialogPrimitive.Content
          {...props}
          className={classNames(
            "fadeIn bg-default fixed left-1/2 top-1/2 z-50 w-full max-w-[22rem] -translate-x-1/2 -translate-y-1/2 rounded-md text-left shadow-xl focus-visible:outline-none sm:align-middle sellular-dialog-box",
            props.size == "xl"
              ? "p-8 sm:max-w-[90rem]"
              : props.size == "lg"
              ? "p-8 sm:max-w-[70rem]"
              ...
              : "p-8 sm:max-w-[35rem]",
            "max-h-[95vh]",
            disableOverlay ? 'border border-default top-[250px]':'',
          ...
          {type === "confirmation" && (
            <div className="flex">
              ...
              <div className="w-full">
                <DialogHeader title={title} subtitle={props.description} />
                <div className={`${disableOverlay ? "" :'flex  space-y-6'}`}>{children}</div>
              </div>
            </div>
          )}
          {!type && children}
...
export function DialogHeader(props: DialogHeaderProps) {
  ...
  return (
    <div>
      <h3 className="leading-20 text-semibold font-cal text-emphasis pb-3 text-base" id="modal-title">
        {props.title}
      </h3>
      {props.subtitle && <div className="text-subtle text-sm mb-3">{props.subtitle}</div>}
    </div>
  );
}
...
export function DialogFooter(props: { children: ReactNode; className?: string; showDivider?: boolean }) {
  return (
    <div className={classNames("bg-default", props.className)}>
      <div
        className={classNames(
          "flex justify-end rtl:space-x-reverse mt-4",
