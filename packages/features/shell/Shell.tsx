...
const Layout = (props: LayoutProps) => {
  ...
  const router = useRouter();
  const { isSelSettings, isSelConfSettings } = router.query
    ...
    <>
      ...
      <div className="flex min-h-screen flex-col">
        ...
        <div className="flex flex-1" data-testid="dashboard-shell">
          {props.SidebarContainer || (!isSellularSettings && !isSellularConfSettings) && <SideBarContainer bannersHeight={bannersHeight} />}
...
type LayoutProps = {
  ...
  isPublic?: boolean;
  isSellularSettings?: boolean;
  isSellularConfSettings?: boolean;
  withoutMain?: boolean;
...
interface NavigationProps {
  isSellularCalendar: boolean;
}

const Navigation = ({ isSellularCalendar }:NavigationProps) => {
  const allowedTabs = ["event_types_page_title", "bookings", "availability"];
  const navigationFiltered = useMemo(() => {
    return desktopNavigationItems.filter((item) => {
      if (isSellularCalendar || item.href.includes("isSellular=true")) {
        return allowedTabs.includes(item.name);
      }
      return true;
    });
  }, []);
  return (
    <>
      <nav className="mt-2 flex-1 md:px-2 lg:mt-6 lg:px-0">
        <div className="sellular-nav">
          {navigationFiltered.map((item) => {
            if (isSellularCalendar && !item.href.includes("isSellular=true")) {
              item.href = `${item.href}?isSellular=true`;
            }
            return (
              <NavigationItem
                key={item.name}
                item={item}
                isSellularCalendar={isSellularCalendar}
              />
            );
          })}
...
const path= item.href?.split('?')?.[0]
  return isChild ? path === router.asPath : router.asPath.startsWith(path);
};
...
const NavigationItem: React.FC<{
  ...
  const { item, isChild,isSellularCalendar } = props;
    ...
    <Fragment>
      <Tooltip side="right" content={t(item.name)} className="lg:hidden">
        <Link
          ...
          className={classNames(
            "[&[aria-current='page']]:bg-emphasis [&[aria-current='page']]:sellular-active-tab sellular-tab text-default group flex items-center py-2 px-3 text-sm font-medium",
            isChild
...
const MobileNavigationItem: React.FC<{
  ...
  return (
    <Link
      key={item.name}
      href={item.href}
      className="[&[aria-current='page']]:text-emphasis hover:text-default text-muted relative my-2 min-w-0 flex-1 overflow-hidden !bg-transparent p-1 text-center text-xs font-medium focus:z-10 sm:text-sm"
      aria-current={current ? "page" : undefined}>
...
function SideBar({ bannersHeight }: SideBarProps) {
  ...
  //hardcoding isSellularCalendar as true to behave calendar as same like iframe even if its outside iframe
  const isSellularCalendar = true

  return (
    <div className="relative">
      <aside
        style={{ maxHeight: `calc(100vh - ${bannersHeight}px)`, top: `${bannersHeight}px` }}
        className="desktop-transparent border-muted fixed left-0 hidden h-full max-h-screen w-14 flex-col overflow-y-auto overflow-x-hidden border-r  md:sticky md:flex lg:w-56 lg:px-4 sellular-aside">
      {isSellularCalendar && <div className="sellular-heading">CALENDAR</div>}
        <div className="flex h-full flex-col justify-between pb-3">
          ...
          <Navigation isSellularCalendar={isSellularCalendar} />
        </div>
        {!isSellularCalendar && (
        <div>
...
export function ShellMain(props: LayoutProps) {
  ...
  return (
    <>
    {(!isSellularSettings && !isSellularConfSettings) &&
      <div
        className={classNames(
          "flex items-center md:mb-6 md:mt-0 px-4",
          ...
          <header
            ...
            <div className={classNames("w-full truncate ltr:mr-4 rtl:ml-4 md:block", props.headerClassName)}>
              {props.heading && (
                <h3
                  className={classNames(
                    "font-cal max-w-28 sm:max-w-72 md:max-w-80 text-emphasis inline truncate text-lg font-semibold tracking-wide sm:text-base md:block xl:max-w-full",
...
export const MobileNavigationMoreItems = () => (
  <ul className="border-subtle mt-2 border">
