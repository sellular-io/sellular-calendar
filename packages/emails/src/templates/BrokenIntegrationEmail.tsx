...
import { WEBAPP_URL , SELLULAR_URL} from "@calcom/lib/constants";
...
const BrokenVideoIntegration = (props: { location: string; eventTypeId?: number | null; t: TFunction; isLinkFromSellularCal?: boolean | null}) => {
  const calendarIframeUrl = props.isLinkFromSellularCal ? `${SELLULAR_URL}/calendar?path=` : ''
  const settingsIframeUrl = props.isLinkFromSellularCal ? `${SELLULAR_URL}/settings/calendar/connected-calendars/?path=` : ''
  return (
    <Trans i18nKey="broken_video_action" t={props.t}>
      ...
      <a
        href={
          props.eventTypeId ? `${calendarIframeUrl}${WEBAPP_URL}/event-types/${props.eventTypeId}${props.isLinkFromSellularCal? `?isSellular=true` : ''}` : `${WEBAPP_URL}/event-types${props.isLinkFromSellularCal? `?isSellular=true` : ''}`
        }>
        change your location on the event type
      </a>
      &nbsp;or try&nbsp;
      <a href={`${settingsIframeUrl}${WEBAPP_URL}/apps/installed${props.isLinkFromSellularCal? `?isSelSettings=true` : ''}`}>removing and adding the app again.</a>
    </Trans>
  );
};
  ...
  if (type === "video") {
    ...
    return (
      <BaseScheduledEmail
        ...
        subtitle={<BrokenVideoIntegration location={location} eventTypeId={calEvent.eventTypeId} t={t} isLinkFromSellularCal={calEvent.isLinkFromSellularCal} />}
        headerType="xCircle"
        {...props}
      />
    );
