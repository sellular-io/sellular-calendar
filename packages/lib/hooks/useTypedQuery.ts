...
export function useTypedQuery<T extends z.AnyZodObject>(schema: T) {
  ...
  const  isSellular  = unparsedQuery?.isSellular;
  ...
  const setQuery = useCallback(
    function setQuery<J extends OutputKeys>(key: J, value: Output[J]) {
      ...
      let newValue = { ...newQuery, [key]: value };
      if(isSellularCalendar) {
        newValue = {...newValue, isSellular: true} as Output
      }
  ...
  function removeByKey(key: OutputOptionalKeys) {
    let { [key]: _, ...newQuery } = parsedQuery;
    if(isSellularCalendar) {
      newQuery = {...newQuery, isSellular: true} as Output
    }
