...
export const getCancelLink = (calEvent: CalendarEvent): string => {
  ...
  if(calEvent.isLinkFromSellularCal) {
  cancelLink.searchParams.append("isLinkFromSellular", String(true));
  }
...
export const getRescheduleLink = (calEvent: CalendarEvent): string => {
  ...
  return `${WEBAPP_URL}/reschedule/${seatUid ? seatUid : Uid}${calEvent.isLinkFromSellularCal? `?isLinkFromSellular=true` : ''}`;
};
