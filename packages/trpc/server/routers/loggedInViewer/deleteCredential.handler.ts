...
export const deleteCredentialHandler = async ({ ctx, input }: DeleteCredentialOptions) => {
  ...
  await updateDefaultConferencingApp(ctx, credential);

  const defaultConferenceCredential = await getDefaultConferenceCredential(ctx);
  const presentDefaultConferenceAppSlug = defaultConferenceCredential.appId;
  const appIdLocationTypeMap: { [key: string]: string } = {
      ...
      if (credential.app?.categories.includes(AppCategories.video)) {
        ...
        const updatedLocations = locations.map((location: { type: string }) => {
          if (location.type.includes(integrationQuery)) {
            return { type: appIdLocationTypeMap[presentDefaultConferenceAppSlug] };
          }
          return location;
        });
...
async function updateDefaultConferencingApp(ctx: any, credential: any) {
  // Make the next possible conference app as default.
  const currentMetadata = userMetadata.parse(ctx.user.metadata);
  const presentDefaultConferenceApp = currentMetadata?.defaultConferencingApp;

  if (credential.app?.slug === presentDefaultConferenceApp?.appSlug) {
    const nextConferenceCredential = await prisma.credential.findFirst({
      where: {
        userId: ctx.user.id,
        type: {
          in: ["google_video", "office365_video", "zoom_video"],
        },
        NOT: {
          id: credential.id,
        },
    ...
    if (!nextConferenceCredential) {
      // This is blocked from UI
      console.log("No default conference app present");
      return;
    }
      ...
      data: {
        metadata: {
          ...
          defaultConferencingApp: {
            appSlug: nextConferenceCredential?.app?.slug,
          },
        },
      },
    });
...
async function getDefaultConferenceCredential(ctx: any) : Promise<any> {
  ...
  const defaultConfAppSlug = userMetadata.parse(userObj?.metadata)?.defaultConferencingApp?.appSlug;
  const defaultConfCredential = await prisma.credential.findFirst({
    where: {
      userId: ctx.user.id,
      type: {
        in: ["google_video", "office365_video", "zoom_video"],
      ...
      app: {
        slug: defaultConfAppSlug,
      }
    },
  });
  return defaultConfCredential;
}
