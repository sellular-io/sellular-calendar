...
export default class GoogleCalendarService implements Calendar {
  ...
  private googleAuth = (credential: CredentialPayload) => {
    console.log("#### lookhere for credential.key");
    console.log(JSON.stringify(credential.key));
    const googleCredentials = googleCredentialSchema.parse(credential.key);
    console.log("#### lookhere for googleCredentials");
    ...
    const refreshAccessToken = async (myGoogleAuth: Awaited<ReturnType<typeof getGoogleAuth>>) => {
      try {
        console.log("#### lookhere while getting access_token googleCredentials.refresh_token");
        console.log(googleCredentials.refresh_token);
        ...
        console.error("### lookhere for error refresh google token");
        console.log(err);
    ...
    return {
      getToken: async () => {
        ...
        console.log("#### lookhere for googleCredentials isExpired");
        console.log(isExpired());
        return !isExpired() && false ? Promise.resolve(myGoogleAuth) : refreshAccessToken(myGoogleAuth);
      },
    };
  };
      ...
      calendar.events.delete(
        ...
        function (err: GoogleCalError | null, event) {
          if (err) {
            ...
            // SEL - Change
            // Resolving this for now as google giving 400 -> URL does not exist on their server
            // /calendar/v3/calendars/addressbook%23contacts%40group.v.calendar.google.com/events/?sendNotifications=false&sendUpdates=none
            // return resolve();
            return reject(err);
          }
